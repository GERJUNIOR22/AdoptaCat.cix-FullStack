# Diagnóstico completo del error de conexión AdoptaCat
...
try {
    ...
    Write-Host "❌ Error verificando backend: $($_.Exception.Message)" -ForegroundColor Red
}
...
## 3. Probar endpoint específico de adopción
...
try {
    $frontendConnection = Test-NetConnection -ComputerName localhost -Port 4200 -WarningAction SilentlyContinue
    ...
    Write-Host "❌ Error verificando frontend: $($_.Exception.Message)" -ForegroundColor Red
}
...
try {
    ...
    Write-Host "✅ Envío de datos simulado exitoso" -ForegroundColor Green
    Write-Host "📝 Respuesta: $($testPost | ConvertTo-Json)" -ForegroundColor White
} catch {
